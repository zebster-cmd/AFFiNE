...
- `prop:views` — `ViewBasicDataType[]`, each `{ id, name, mode }`. Table view: `mode: 'table'`. Kanban view (`mode: 'kanban'`): additionally `groupBy` — the nested `GroupBy` **object** `{ type: 'groupBy', columnId, name, hideEmpty?, sort? }` (see `blocksuite/affine/data-view/src/core/common/types.ts:1-9` and `.../view-presets/kanban/define.ts:20`), NOT a flat column-id string; read the referenced column from `groupBy.columnId`. Also `groupProperties: GroupProperty[]` (array order = group display order; each has `key` = the group's raw cell value, e.g. a select option id, and `manuallyCardSort: string[]` = card row-id order within the group), `header: { titleColumn, iconColumn, coverColumn }`. The tool-facing JSON projection (`ViewJSON.groupByColumnId`) flattens this back to just the column-id string.
